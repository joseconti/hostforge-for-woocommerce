...
<?php if ( $admin_template ) { ?>
	<p>
		...
		printf( esc_html__( 'You have received a Subscription order from your previous subscriber %1$s %2$s. The Subscription order is as follows:', 'sumosubscriptions' ), esc_html( $order->get_billing_first_name() ), esc_html( $order->get_billing_last_name() ) ) ;
		?>
	</p>
<?php } else { ?>
	<p>
		...
		printf( esc_html__( 'You have placed another subscription order on %s. The Subscription order is as follows:', 'sumosubscriptions' ), esc_html( get_option( 'blogname' ) ) ) ;
		?>
	</p>
<?php } ?>
