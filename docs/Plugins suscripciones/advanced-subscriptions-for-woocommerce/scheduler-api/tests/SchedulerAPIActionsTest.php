...
class Dummy_Payment_Scheduler_Actions {
    public $actions;
...
class Dummy_Lifecycle_Scheduler_Actions {
    public $actions;
    public $last_group = null;
    public function __construct( $actions = array() ) { $this->actions = $actions; }
...
class Dummy_Notification_Scheduler_Actions {
    public $actions;
    public $last_group = null;
    public function __construct( $actions = array() ) { $this->actions = $actions; }
...
class SchedulerAPIActionsTest extends TestCase {
    ...
    public function test_get_scheduled_actions_combines_modules_and_forwards_group() {
        $payment = new Dummy_Payment_Scheduler_Actions( array( 'next_payment' => 'p' ) );
        $lifecycle = new Dummy_Lifecycle_Scheduler_Actions( array( 'expiration' => 'e' ) );
        $notification = new Dummy_Notification_Scheduler_Actions();
    ...
    public function test_get_all_last_scheduled_actions_includes_notifications() {
        $payment = new Dummy_Payment_Scheduler_Actions( array( 'next_payment' => 'p' ) );
        $lifecycle = new Dummy_Lifecycle_Scheduler_Actions( array( 'expiration' => 'e' ) );
        $notification = new Dummy_Notification_Scheduler_Actions( array( 'trial_end' => 'n' ) );
        $this->inject_modules( $payment, $lifecycle, $notification );
        $subscription = new stdClass();
        $result = ASWC_Scheduler_API::get_all_last_scheduled_subscription_actions( $subscription, array( 'trial_end' ), 'group-b' );
        $this->assertSame( array( 'next_payment' => 'p', 'expiration' => 'e', 'trial_end' => 'n' ), $result );
    ...
    public function test_get_last_scheduled_actions_combines_modules_and_forwards_group() {
        $payment      = new Dummy_Payment_Scheduler_Actions( array( 'next_payment' => 'p2' ) );
        $lifecycle    = new Dummy_Lifecycle_Scheduler_Actions( array( 'expiration' => 'e2' ) );
        $notification = new Dummy_Notification_Scheduler_Actions();
        $this->inject_modules( $payment, $lifecycle, $notification );
        $subscription = new stdClass();
        $result       = ASWC_Scheduler_API::get_last_scheduled_subscription_actions( $subscription, 'group-c' );
        $this->assertSame( array( 'next_payment' => 'p2', 'expiration' => 'e2' ), $result );
        $this->assertSame( 'group-c', $payment->last_group );
        $this->assertSame( 'group-c', $lifecycle->last_group );
    }

    public function test_get_all_scheduled_actions_includes_notifications() {
        $payment      = new Dummy_Payment_Scheduler_Actions( array( 'next_payment' => 'p3' ) );
        $lifecycle    = new Dummy_Lifecycle_Scheduler_Actions( array( 'expiration' => 'e3' ) );
        $notification = new Dummy_Notification_Scheduler_Actions( array( 'trial_end' => 'n3' ) );
        $this->inject_modules( $payment, $lifecycle, $notification );
        $subscription = new stdClass();
        $result       = ASWC_Scheduler_API::get_all_scheduled_subscription_actions( $subscription, array( 'trial_end' ), 'group-d' );
        $this->assertSame( array( 'next_payment' => 'p3', 'expiration' => 'e3', 'trial_end' => 'n3' ), $result );
        $this->assertSame( 'group-d', $payment->last_group );
        $this->assertSame( 'group-d', $lifecycle->last_group );
        $this->assertSame( 'group-d', $notification->last_group );
    }
}
