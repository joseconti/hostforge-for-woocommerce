...
function App(props) {
    ...
    const steps = [ __( 'General Settings', 'advanced-subscriptions-for-woocommerce' ), __( 'Create Subscription', 'advanced-subscriptions-for-woocommerce' ), __( 'Subscription Payment Gateway Configuration', 'advanced-subscriptions-for-woocommerce' ), __( 'Final Step', 'advanced-subscriptions-for-woocommerce' )];
    ...
    const getStepContent = (stepIndex) => {
        switch (stepIndex) {
            ...
            return <FinalStep />;
            case 4:
                return <h1>{__( 'Thanks for your details', 'advanced-subscriptions-for-woocommerce' )}</h1>;
            default:
                return __( 'Unknown stepIndex', 'advanced-subscriptions-for-woocommerce' );
        }
    }
    const handleNext = () => {
        ...
        if( activeStep == 1 ){
            ...
            if( state['SubscriptionNumber'] < 0 || state['ProductPrice'] < 0){
                ...
                alert(__('Negative Subscription Interval Or Price is not allowed','advanced-subscriptions-for-woocommerce'));
    ...
    const handleFormSubmit = (e) => {
        ...
        const user = {
            ...state,
            'action': 'aswc_save_settings_filter',
            nonce: frontend_ajax_object.aswc_react_nonce,   // pass the nonce here
        };
    ...
    return (
        <Context.Provider value={{
            ...
            <div className="aswcMsfWrapper">
                <Stepper activeStep={activeStep} steps={steps}/>
                <div className="aswcHeadingWrap">
                    <h2>{__( 'Welcome to Advanced Subscriptions For WooCommerce', 'advanced-subscriptions-for-woocommerce' ) }</h2>
                    <h3>{__('Complete steps to start selling subscriptions','advanced-subscriptions-for-woocommerce') }</h3>
                </div>
                <Container maxWidth="sm">
                    <form className="aswcMsf">
                        <Typography className={classes.instructions}>
                            {(loading) ? <CircularProgress className="aswcCircularProgress" /> :getStepContent(activeStep)}
                        </Typography>
                        <div className="aswcButtonWrap">
