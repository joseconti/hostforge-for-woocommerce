...
class ASWC_Admin {
	...
	public function aswc_admin_cancel_susbcription() {
		...
		// Only proceed if we're actually trying to cancel (not just viewing the page).
		if ( ! isset( $_GET['aswc_subscription_status_admin'] ) || ! isset( $_GET['aswc_subscription_id'] ) ) {
			return;
		}
		...
		if ( ! isset( $_GET['_wpnonce'] ) ) {
			wp_die( esc_html__( 'Security check failed: Missing nonce.', 'advanced-subscriptions-for-woocommerce' ), 403 );
		}
	...
	public function aswc_custom_product_fields_for_subscription_box() {
		...
		<div id="aswc_subscription_box_product_target_section" class="panel woocommerce_options_panel hidden">
			...
			<p class="form-field aswc_subscription_box_price_field ">
				...
				<?php
					$description_text = __( 'Set a fixed price for the entire subscription bundle. This price will be charged per renewal period, regardless of how many products are included. Example: $29.99 for a monthly box with multiple products.', 'advanced-subscriptions-for-woocommerce' );
					echo wp_kses_post( wc_help_tip( $description_text ) ); // WPCS: XSS ok.
				?>
			</p>
			<p class="form-field aswc_manage_subscription_box_price_field aswc_subscription_box_price_field_pro <?php echo esc_attr( $pro_group_tag ); ?>">
				...
				<?php
					$description_text = __( 'When enabled, the subscription price will be automatically calculated based on the sum of all selected products\' prices. If disabled, use the fixed price set above. Example: If checked and products cost $10 + $15 + $5, subscription price = $30.', 'advanced-subscriptions-for-woocommerce' );
					echo wp_kses_post( wc_help_tip( $description_text ) ); // WPCS: XSS ok.
				?>
			</p>
			...
			<?php
				$description_text = __( 'Set how often the subscription will renew and charge the customer. Example: Enter "1" and select "Month" for monthly renewals, or "2" and select "Weeks" for billing every 2 weeks. This determines your recurring payment schedule.', 'advanced-subscriptions-for-woocommerce' );
				echo wp_kses_post( wc_help_tip( $description_text ) ); // WPCS: XSS ok.
			?>
			</p>
			...
			<?php
				$description_text = __( 'Set when the subscription will automatically end and stop billing. Leave empty for unlimited subscriptions that continue until canceled. Example: Enter "12" and select "Months" for a subscription that expires after 1 year. After expiry, no more charges will occur.', 'advanced-subscriptions-for-woocommerce' );
				echo wp_kses_post( wc_help_tip( $description_text ) ); // WPCS: XSS ok.
			?>
			</p>
			...
			<strong><?php esc_html_e( 'Setup For Multi-Product Subscription', 'advanced-subscriptions-for-woocommerce' ); ?></strong>
				...
				<?php
					$description_text = __( 'Choose which products will be included in this subscription bundle. Select "Specific Products" to manually pick individual products, or "Specific Categories" to include all products from chosen categories. The customer will receive all selected items with each subscription delivery.', 'advanced-subscriptions-for-woocommerce' );
					echo wp_kses_post( wc_help_tip( $description_text ) ); // WPCS: XSS ok.
				?>
				</p>
				...
				<?php
					$description_text = __( 'Search and select the specific products that will be included in this subscription box. You can select multiple products. These exact products will be delivered to the customer with each renewal. Example: Select "Coffee Beans", "Cookies", and "Tea" for a monthly gourmet box.', 'advanced-subscriptions-for-woocommerce' );
					echo wp_kses_post( wc_help_tip( $description_text ) ); // WPCS: XSS ok.
				?>
				</p>
				...
				<?php
					$description_text = __( 'Select product categories to automatically include all products from those categories in the subscription. This is useful for dynamic subscriptions where products may change. Example: Select "Organic Vegetables" to include all vegetable products, even new ones added later.', 'advanced-subscriptions-for-woocommerce' );
					echo wp_kses_post( wc_help_tip( $description_text ) ); // WPCS: XSS ok.
				?>
								</p>
				...
				<?php
					$description_text = __( 'Enter a custom label or title for this subscription step/plan. This text will be displayed to customers when they view the subscription options. Example: "Monthly Starter Box" or "Premium Annual Package". This helps customers identify different subscription tiers.', 'advanced-subscriptions-for-woocommerce' );
					echo wp_kses_post( wc_help_tip( $description_text ) ); // WPCS: XSS ok.
				?>
								</p>
	...
	public function aswc_admin_reactivate_onhold_susbcription() {
		...
		// Only proceed if we're actually trying to reactivate.
		if ( ! isset( $_GET['aswc_subscription_status_admin_reactivate'] ) || ! isset( $_GET['aswc_subscription_id'] ) ) {
			return;
		}
		...
		$nonce = sanitize_text_field( wp_unslash( $_GET['_wpnonce'] ) );
		if ( ! wp_verify_nonce( $nonce, 'aswc_reactivate_subscription_' . $aswc_subscription_id ) ) {
			wp_die( esc_html__( 'Security check failed.', 'advanced-subscriptions-for-woocommerce' ), 403 );
		}
