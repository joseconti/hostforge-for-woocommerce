<?php
/**
 * License Manager for Advanced Subscriptions for WooCommerce
 *
...
class ASWC_License {
	...
	public function __construct() {
		...
		if ( ! defined( 'ASWC_LICENSE_API' ) || ! defined( 'ASWC_PLUGIN_FILE' ) || ! defined( 'ASWC_VERSION' ) || ! defined( 'ASWC_LICENSE_ITEM_NAME' ) || ! defined( 'ASWC_LICENSE_PREFIX' ) ) {
			return;
		}

		$this->api_url       = trailingslashit( ASWC_LICENSE_API );
		$this->slug          = plugin_basename( ASWC_PLUGIN_FILE );
		$this->plugin_file   = ASWC_PLUGIN_FILE;
		$this->name          = 'advanced-subscriptions-for-woocommerce';
		$this->version       = ASWC_VERSION;
		$this->wp_override   = false;
		$this->item_name     = ASWC_LICENSE_ITEM_NAME;
		$this->prefix        = ASWC_LICENSE_PREFIX;
		$this->license_title = __( 'Advanced Subscriptions License', 'advanced-subscriptions-for-woocommerce' );
	...
	public function admin_notice() {
		// Only show on ASWC admin pages and WooCommerce pages.
		$screen = get_current_screen();
		if ( ! $screen ) {
			return;
		}

		// Show on subscription pages and WooCommerce settings.
		$show_on_screens = array(
			'aswc_subscriptions',
			'edit-aswc_subscriptions',
			'woocommerce_page_wc-orders--aswc_subscriptions',
			'woocommerce_page_wc-settings',
		);

		$show = false;
		foreach ( $show_on_screens as $screen_id ) {
			if ( strpos( $screen->id, $screen_id ) !== false ) {
				$show = true;
				break;
			}
		}
		...
		<div class="notice notice-warning">
			<p>
				<strong><?php esc_html_e( 'Advanced Subscriptions for WooCommerce', 'advanced-subscriptions-for-woocommerce' ); ?></strong>:
				<?php esc_html_e( 'Please enter your license key to enable automatic updates.', 'advanced-subscriptions-for-woocommerce' ); ?>
				<a href="<?php echo esc_url( admin_url( 'admin.php?page=wc-settings&tab=aswc_subscriptions&section=license' ) ); ?>">
					<?php esc_html_e( 'Enter License Key', 'advanced-subscriptions-for-woocommerce' ); ?>
				</a>
			</p>
		</div>
		<?php
	...
	public function activate_license() {
		...
		if ( 200 !== $response_code ) {
			return __( 'Error: Could not connect to license server.', 'advanced-subscriptions-for-woocommerce' );
		}

		$body = wp_remote_retrieve_body( $response );
		if ( empty( $body ) ) {
			return __( 'Error: Empty response from license server.', 'advanced-subscriptions-for-woocommerce' );
		}

		$license_data = json_decode( $body );

		if ( ! is_object( $license_data ) ) {
			return __( 'Error: Invalid response from license server.', 'advanced-subscriptions-for-woocommerce' );
		}

		if ( ! isset( $license_data->activated ) || false === $license_data->activated ) {
			$message = ! empty( $license_data->error ) ? $license_data->error : __( 'Error: Could not activate license.', 'advanced-subscriptions-for-woocommerce' );
			delete_option( $this->prefix . '_license_status' );
			return $message;
		}
	...
	public function plugins_api_filter( $data, $action = '', $args = null ) {
		...
		$cache_key = 'aswc_api_request_' . substr( md5( wp_json_encode( $this->item_name ) ), 0, 15 );
	...
	public function in_plugin_update_message( $args ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.Found
		...
		if ( false !== $version_info && is_object( $version_info ) && isset( $version_info->new_version ) ) {
			...
			if ( version_compare( $this->version, $version_info->new_version, '<' ) && ! empty( $version_info->is_major ) ) {
				$upgrade_notice = '<span class="aswc_plugin_upgrade_notice"> ';
				...
				if ( ! empty( $version_info->major_log ) ) {
					...
					$upgrade_notice .= sprintf(
						...
						esc_html__( '%s is a major update. We recommend creating a full backup before updating.', 'advanced-subscriptions-for-woocommerce' ),
				...
				echo '<style type="text/css">
					...
					.aswc_plugin_upgrade_notice:before {
						content: "\f348";
	...
	public static function get_license_status() {
		if ( ! defined( 'ASWC_LICENSE_PREFIX' ) ) {
			return '';
		}
		return get_option( ASWC_LICENSE_PREFIX . '_license_status', '' );
	}
	...
	public static function get_license_key() {
		...
		return get_option( ASWC_LICENSE_PREFIX . '_license_key', '' );
	}
}
