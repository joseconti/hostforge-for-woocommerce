					...
					if ( isset( $ipn_args['subscr_id'] ) && 'pending' === strtolower( $ipn_args['payment_status'] ) && 'echeck' === strtolower( $ipn_args['payment_type'] ) ) {
						...
						foreach ( $subscriptions as $subscription_id ) {
							...
							if ( $subscription->get_status() === 'pending' || ( ! $last_order && $order->has_status( $valid_order_statuses ) ) ) {
								...
								YITH_WC_Activity()->add_activity( $subscription_id, 'suspended', 'success', $order_id, __( 'Subscription has been suspended because of pending payment for the echeck payment type', 'yith-woocommerce-subscription' ) );

								$last_order->add_order_note( __( 'YSBS - Subscription has been suspended because of pending payment for the echeck payment type', 'yith-woocommerce-subscription' ) );
						...
						if ( $paypal_sub_id != $order_sub_id ) { // phpcs:ignore
							...
							foreach ( $subscriptions as $subscription_id ) {
								...
								if ( $failed_attemp >= ( $max_failed_attempts - 1 ) ) {
									...
									$subscription->update_status( 'suspended', 'paypal' );
									YITH_WC_Activity()->add_activity( $subscription_id, 'suspended', 'success', $order_id, __( 'Subscription has been suspended due to the PayPal IPN message received: recurring_payment_suspended_due_to_max_failed_payment', 'yith-woocommerce-subscription' ) );
