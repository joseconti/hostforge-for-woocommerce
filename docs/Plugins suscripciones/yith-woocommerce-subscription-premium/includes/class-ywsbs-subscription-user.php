		...
		public static function has_subscription( $user_id, $product_id, $status = '' ) {
			...
			if ( $subscriptions ) {
				foreach ( $subscriptions as $subscription ) {
					$post = $subscription->get_post();
					if ( ( $post && 'trash' !== $post->post_status ) && ( in_array( $subscription->get_product_id(), $product_ids, true ) || in_array( $subscription->get_variation_id(), $product_ids, true ) ) && ( ! $status || ( $status && $subscription->has_status( $status ) ) ) ) {
