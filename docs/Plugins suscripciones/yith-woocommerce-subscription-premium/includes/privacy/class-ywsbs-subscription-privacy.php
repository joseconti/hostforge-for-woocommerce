			...
			if ( $subscriptions ) {
				foreach ( $subscriptions as $subscription ) {
					if ( apply_filters( 'ywsbs_privacy_erase_personal_data', $erasure_enabled, $subscription ) ) {
						...
						$response['messages'][]    = sprintf( esc_html_x( 'Personal data removed from subscription %s.', 'placeholder subscription number', 'yith-woocommerce-subscription' ), $subscription->ID );
