...
if ( ! class_exists( 'YITH_WC_Customer_Subscription_Expired' ) ) {
	...
	class YITH_WC_Customer_Subscription_Expired extends YITH_WC_Customer_Subscription {
		...
		public function __construct() {
			...
			$this->description = __( 'This email is sent to the customer when the subscription expires.', 'yith-woocommerce-subscription' );
		...
		protected function get_dummy_subscription() {
			...
			$subscription = parent::get_dummy_subscription();

			$subscription->populate_prop( 'status', 'expired' );
